...
datetimepicker:function(){
    this.refs['datetime-picker'].binding(this.refs['demodate']);
}
```
# Some Tips
控件<DateTimePicker />目前的属性有：
- yearRange 值为数组，表示起始与结束年份，不填写时，自动以当前年份上下2年为范围，示例：
```
<DateTimePicker yearRange={[2011,2018]}/>
```
- timeEnable 是否启用小时与分钟选择，默认启用，如果要禁用，请设为false，示例
```
<DateTimePicker yearRange={[2011,2018]} timeEnable={false}/>
```

3.目前本控件的返回格式是：yyyy-mm-dd HH:ii这种格式
