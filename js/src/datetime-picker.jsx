...
define('datetime-picker',['react','jquery'],function(React,$){
    ...
    var SwiperWheelComponent = React.createClass({
        ...
        render(){
            ...
            for(let i=this.props.min;i<=this.props.max;i++){
                 h.push(<li key={this.props.ckey+'-'+i}>{i+this.props.unit}</li>);
            }
            return (
                   <div className="dt-swiper-container" style={{'height':this.props.height+'px'}}>
                       <div className="dt-swiper"  style={{'height':this.props.height+'px'}}>
                           <ul id={this.props.ckey+'Wheel'} >{h}
                           </ul>
    ...
    return React.createClass({
         ...
         *
         */
        binding:function(inputNode,defaultDate,cb){
        ...
        save(){
            ...
            if(selectedDateJson['year'] !== undefined &&
                selectedDateJson['month']!== undefined &&
                selectedDateJson['date']!== undefined &&
                selectedDateJson['hour']!== undefined &&
                selectedDateJson['minute']!== undefined){
                var m=0,h=0;
        ...
        render(){
            return(
                <div className='rdp-datetime-picker animated-fast datetime-hide' ref="datetime-picker">
                    <div className="btns">
                        <button className="btn" onClick={this.save}>确定</button>
                        <button className="btn" onClick={this.closePicker}>取消</button>
                    </div>
                    <div className="datetime-panel clearfix">
                        <div className="panel-item year-panel">
                            <SwiperWheelComponent onSelected={this.selectItem} ckey="year" unit="年" ref="year" min={this.props.yearRange[0]} max={this.props.yearRange[1]} value={this.state.selected['year']}/>
                        </div>
                        <div className="panel-item month-panel">
                            <SwiperWheelComponent onSelected={this.selectItem} ckey="month"  unit="月" ref="month" min={this.state.month[0]} max={this.state.month[1]} value={this.state.selected['month']}/>
                        </div>
                        <div className="panel-item date-panel">
                            <SwiperWheelComponent onSelected={this.selectItem} ckey="date"  unit="日" ref="date" min={this.state.date[0]} max={this.state.date[1]} value={this.state.selected['date']}/>
                        </div>
                        <div className={this.props.timeEnable?"panel-item hour-panel":'datetime-hide'}>
                            <SwiperWheelComponent onSelected={this.selectItem} ckey="hour" ref="hour" min="0" max="23" value={this.state.selected['hour']}/>
                        </div>
                        <div className={this.props.timeEnable?"panel-item minute-panel":'datetime-hide'}>
                            <SwiperWheelComponent onSelected={this.selectItem} ckey="minute" ref="minute" min="0" max="59" value={this.state.selected['minute']}/>
                        </div>
                    </div>
                </div>
            )
